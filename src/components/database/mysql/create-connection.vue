...
<template>
  <common-form ref="commonFormRef" v-if="dataInitCompleted" v-model="formData" :rules="formRules">
        ...
        <el-row>
          <el-col :span="13">
            <el-form-item label="连接方式" prop="detail.connectionType">
              <el-select v-model="formData.detail.connectionType" style="width: 100%">
                <el-option v-for="item in ConnectionTypes" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="9" :offset="2">
            <el-form-item label="驱动" prop="detail.driver">
              <el-select v-model="formData.detail.driver" @change="onChangeUrlByDriver">
                <el-option v-for="item in DatabaseDrivers.mysql" :key="item.value" :value="item.value"
                  :label="item.label" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="13">
            <el-form-item label="认证方式" prop="detail.authType">
              <el-select v-model="formData.detail.authType" style="width: 100%">
                <el-option v-for="item in AuthenticationTypes" :key="item.value" :value="item.value"
                  :label="item.label" />
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="9" :offset="2">
            <el-form-item label="保存密码" prop="detail.savePwdType">
              <el-select v-model="formData.detail.savePwdType">
                <el-option v-for="item in SavePasswordTypes" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>

        <el-form-item v-if="formData.detail.connectionType === 'only_url'" label="URL" prop="detail.url">
          <el-input v-model="formData.detail.url" type="textarea" resize="none" :autosize="{
            minRows: 1,
            maxRows: 5
          }" />
        </el-form-item>
      ...
      <el-tab-pane name="advanced" label="高级">
        <el-row>
          <el-col :span="11">
            <el-form-item label="时区" prop="detail.timeZone">
              <el-select-v2 v-model="formData.detail.timeZone" :options="timeZonesOptions" placeholder=" " clearable
                filterable :persistent="false" no-data-text="没有可匹配项" />
            </el-form-item>
          </el-col>
          <el-col :span="11" :offset="2">
            <el-form-item label="编码" prop="detail.charset">
              <el-select v-model="formData.detail.charset" filterable no-match-text="没有可匹配项">
                <el-option v-for="item in CharsetTypes" :key="item.value" :value="item.value" :label="item.label" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="13" style="display: flex;align-items: center;margin-bottom: 18px">
            ...
            <el-input-number v-model="formData.detail.connectionIntervalTime" :controls="false"
              class="el-input-number__text-left" style="margin-left: 10px;width: 80px"
        ...
        <el-row>
          <el-col :span="13" style="display: flex;align-items: center">
            ...
            <el-input-number v-model="formData.detail.autoBreakTime" :controls="false" class="el-input-number__text-left"
              style="margin-left: 10px;width: 80px" :disabled="!formData.detail.autoBreak" />
            <span style="margin-left: 10px">秒</span>
          </el-col>
        </el-row>
      </el-tab-pane>
