...
export const useContextmenu = (
    ...
    comRef: Ref<HTMLElement | undefined>
) => {
    ...
    function calcTopMenuPosition(event: MouseEvent) {
        ...
        const comWidth = comRef.value?.clientWidth || 0, comHeight = comRef.value?.clientHeight || 0
    ...
    function calcChildMenuPosition(event: any) {
        ...
        const body = document.body
        const bodyWidth = body.clientWidth, bodyHeight = body.clientHeight
        const parentWidth = _parent.clientWidth, parentHeight = _parent.clientHeight
        const comWidth = comRef.value?.clientWidth || 0, comHeight = comRef.value?.clientHeight || 0

        let x = clientX + parentWidth - 3, y = clientY - 7
